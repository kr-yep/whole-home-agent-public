...
def _verbalizer_system(name: str) -> str:
    return (
        ...
        "  → 報告主人，包包裡面{name}收著鑰匙和錢包兩樣呢。\n"
        "  {\"status\": \"CONTENTS\", \"container\": \"抽屜\", \"items\": []}\n"
        "  → 抽屜裡面現在是空的喔，{name}的記錄上沒有任何東西收在那裡。\n"
        "  {\"status\": \"TIMELINE\", \"text\": \"這段固定重播的第 1 筆記錄，顯示鑰匙進入包包；沒有可換算的影片秒數。\"}\n"
        "  → 是第 1 筆記錄的時候呢，主人的鑰匙就是在那時進到包包裡的。{name}這邊沒有可以換算的影片秒數，只能報告到這裡。\n"
        ...
        "FOUND、CONTENTS、TIMELINE 都是查到了，要把裡面的內容講出來。\n"
        "TIMELINE 的 text 已經是查到的答案，用{name}的語氣重講一次，不要改動裡面的數字，"
        "也不要因為它沒有秒數就當成沒查到。\n"
        "只有 UNKNOWN 之類找不到的狀態才是不知道，那時要老實說，"
        "不要給一個聽起來像答案的答案。\n"
