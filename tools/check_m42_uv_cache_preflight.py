"""Run the single bounded M42 uv cache-path preflight; never build a package."""
...
CONTRACT = ROOT / "configs" / "evaluation" / "m42-uv-cache-path-preflight-v1.toml"
TARGET_RELATIVE = Path(".tmp") / "m42-uv-cache"
...
def validate_new_cache_root(cache_root: Path, root: Path = ROOT) -> list[str]:
    failures: list[str] = []
...
def _parse_uv_version(output: str, pattern: str) -> str | None:
    match = re.match(pattern, output.strip())
    return match.group(1) if match else None


def _write_probe(cache_root: Path, relative_path: str, payload: str) -> tuple[str, bool]:
...
def run_preflight(
    ...
    *,
    ...
    failures = validate_new_cache_root(cache_root, root)
    version_output = ""
    version = None
    elapsed_ms = 0.0
    stdout = ""
    ...
    if not failures:
        try:
            version_process = runner(
                ...
                text=True,
                shell=False,
                env=sanitized_environment(source_environment or os.environ, cache_root),
            )
            version_output = version_process.stdout.strip()
            version = _parse_uv_version(version_output, contract["uv"]["version_parse_regex"])
    ...
    if not failures:
        ...
        if Path(stdout).resolve(strict=False) != cache_root.resolve(strict=False):
        ...
        if not cache_root.is_dir() or cache_root.is_symlink():
            failures.append("CACHE_NOT_INITIALIZED")

    if not failures:
        try:
            probe_hash, probe_removed = _write_probe(
                cache_root,
                contract["writability_probe"]["relative_path"],
                contract["writability_probe"]["payload"],
            )
        except OSError:
            failures.append("CACHE_NOT_WRITABLE")
        expected_hash = sha256_bytes(contract["writability_probe"]["payload"].encode("ascii"))
        if probe_hash != expected_hash or not probe_removed:
            failures.append("CACHE_PROBE")
    ...
    return {
        ...
        "uv": {
            "version": version,
            "version_output_present": bool(version_output),
            "cache_command_exit_code": exit_code,
            "cache_command_elapsed_ms": round(elapsed_ms, 3),
            "stdout_matches_exact_target": bool(stdout)
            and Path(stdout).resolve(strict=False) == cache_root.resolve(strict=False),
        ...
        "cache": {
            ...
            "initialized": cache_root.is_dir(),
            "write_probe_sha256": probe_hash,
            "write_probe_removed": probe_removed,
        },
        "network": {
        ...
        "evidence_limit": "one explicit cache-path initialization and write probe only; no default-cache root cause, package, network-attempt, teammate, public-CI, or operation claim",
    }
