...
Freeze exactly one detector-replacement candidate whose published design and local artifact contract give a falsifiable reason to improve both low-score and localization failures. Screen only `3518_unscrew_bottle` development first with the unchanged `0.60` recall, `100 ms` p95, and `1 GiB` VRAM bounds; do not open reserved validation unless every development gate passes. Do not spend another run on threshold-only calibration, create movement candidates, rerun/tune on VISOR `P14_05`, add live capture, use private household data, add cloud egress/credentials, control devices, or enable action capability. Preserve greedy-IoU tracker replacement as a required later co-gate because its mask-box oracle produced 16 ID switches.
