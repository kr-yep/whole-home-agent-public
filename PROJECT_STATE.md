...
**Checkpoint:** `PUBLIC-B1-DESIGN-001`
...
| `PUB-DIR-005` | Keep B0 as the only claim-commit/query core; make prerecorded perception a replaceable candidate-producing adapter | Active as a bounded design direction; B1 remains unimplemented |
...
- The B1 architecture is documented but not adopted or implemented; no video-source/perception adapter or frozen indoor evaluation set exists.
...
Review the proposed B0 → B1 plan. With a separate explicit bounded implementation direction, the next code slice may define only the candidate-source seam, a synthetic fake adapter, and contract tests while preserving the full B0 suite. Do not connect a live camera, process household data, add credentials, or implement device action under this checkpoint.
