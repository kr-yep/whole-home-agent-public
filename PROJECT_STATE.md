...
Establish one license-compatible consecutive prerecorded public clip, separate from the frozen VISOR test, so the existing motion-plus-periodic scheduler can be evaluated with the full-frame FPN candidate on event recall, avoided detector calls, p95 latency, and VRAM. Do not run more tile variants on `P14_05`, add live capture, use private household data, add cloud egress/credentials, control devices, or enable action capability.
