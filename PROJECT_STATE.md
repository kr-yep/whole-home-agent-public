...
Localize the development-only M10 failure without changing thresholds or opening the reserved VOST validation source: separate confidence misses, localization-IoU misses, target-size strata, absent/void frames, and track-association failures using retained outputs or one instrumented development rerun. Use the result only to choose whether a bounded detector/tracker replacement deserves a new gate. Do not create movement candidates, rerun/tune on VISOR `P14_05`, add live capture, use private household data, add cloud egress/credentials, control devices, or enable action capability.
