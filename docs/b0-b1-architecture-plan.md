...
3. **B1-2 — baseline perception：完成 synthetic smoke 與 bounded public detector screen。** RGB detector、clip-local IoU tracker、binding 與 conservative relation rules 已接通；VISOR paired baseline 選出 full-frame RetinaNet-FPN 候選；後續 RF-DETR Small development screen 雖提高有限序列匹配數但未達 recall gate，已停止且未進入 claim path。D-FINE Small 只通過生成影像的工程相容與成本檢查；社群轉換權重未驗證與作者 artifact 等價，因此沒有取得 VOST development screen 或 product path 資格。
